(function(){var TIMEOUT_PLAYER_OFFLINE = 5 * 1000;
...
/*        window.intervalPlayerOffline = setInterval(function () {
            var previousPosition = prevPosition.get(),
                currentPosition = position.get();

            if (previousPosition && currentPosition) {
                if (previousPosition.lat == currentPosition.lat &&
    ...
    Meteor.startup(function () {
        ...
        Meteor.setInterval(function () {
            ...
            users.forEach(function(user) {
                console.log("Checking markers of user " + user);
                var lastLogin = user.resume.when;
                if(Date.now() - lastLogin >= TIMEOUT_PLAYER_OFFLINE) {
                    Meteor.call("deletePlayerMarkers", user._id);
                    console.log("SERVER delete markers of user " + user._id)
                }
            })
        }, TIMEOUT_PLAYER_OFFLINE);
    });
...
Meteor.methods({
    ...
    deletePlayerMarkers: function (userId) {
        var markers = Markers.findAll({owner: userId});
        markers.forEach(function(marker) {
            Markers.remove({id: marker._id});
        })
    },
